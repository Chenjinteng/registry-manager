...
- **只需要填一个镜像名**。目标引用自动补全为「本仓库地址 + 源镜像路径」，
  例如填 `library/alpine:3.19`，目标就是 `192.0.2.10:10001/library/alpine:3.19`。
- **目的端主机固定来自配置**，页面上没有任何地方能改它 —— 工具一次只管理一个 registry，
  不存在"配成 push 到另一个 registry"的可能。（目标仓库字段含 `:` 会被直接拒绝。）
