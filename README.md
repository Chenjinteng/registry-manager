...
这个参数**同时管住两个下载器**：`corepack`（下载 pnpm 本身，只认 `COREPACK_NPM_REGISTRY`，
...
- **拉取成功后会自动重新盘点一次**：列表页有一份 TTL 快照，不刷新的话会"拉完切回去搜不到刚拉的东西"。
  任务一到 `succeeded`，服务端立刻作废快照并在后台重建；连拉多个只跑一轮（若跑的过程中又有任务完成，
  跑完再补一轮），且**盘点失败不影响拉取结果**。失败 / 取消不触发 —— 它们不会留下新的 tag。
