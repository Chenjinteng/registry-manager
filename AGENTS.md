...
- 结构：`header`（sticky 顶栏）→ `main` → **顶部横向 `Segmented` 导航** → 内容区。
  应用内导航在**顶部**，不要改成左侧栏。
- **导航顺序按使用频率排，不按功能分类**：`镜像列表 → 镜像拉取 → 镜像热度 → 凭据/代理/设置`。
  最常来的两件事（看一眼有什么、搬一个进来）必须排在最前面一屏够得着；热度是"回头查账"
  时才来的。**不要**按"统计类放一起""管理类放一起"这类分类学去重排 —— 顺序只写在
  `App.tsx` 的 `NAV_ITEMS` 里，改错了不报任何错，只有 `verify:layout` 里那条 DOM 顺序断言挡得住。
- **表格里的进度条必须固定宽度。** 外层 `Space` 是 `width: auto`，会缩到最宽的子元素 ——
  也就是条子下面那行"128 MiB / 128 MiB"；`Progress` 默认占满容器，于是**文字长短直接决定条子长短**，
  看着像每行进度不一样（真实反馈："这些镜像拉取的进度条的长是不一致的"）。
  实测修前 `[111, 96, 110, 107, 101, 101]`、修后全为 168。宽度写在 `app.css` 的 `.pull-progress-bar`，
  详情里用 `--fill` 占满整行。`verify:layout` 量所有行的渲染宽度是否一致 ——
  这条只有量渲染结果才看得出来，读代码完全合理。
...
`scripts/verify-pull.mjs`（`pnpm verify:pull`）除了跑通真实拉取流程，还钉**终态回调
`onSettled` 的三条契约**：非终态不通知、排队中被取消也通知、成功通知一次。
这个回调是"拉完镜像切回列表搜不到东西"的修复入口（见下），所以它挂掉必须能被发现。
**它的回调是故意抛错的** —— 一次同时钉住"通知失败不能影响任务结果"：
实现里漏掉 try/catch 的话，这个脚本会直接崩，而不是静默通过（已验证）。
- 别为 `#settle` 里那个 `job.settled` 守卫写断言：`cancel()` 对已终态任务是**提前 return** 的，
  写出来的 `cancel(已完成任务)` 根本进不到 `#settle`，**看着在测、其实什么都没测**（真写过一次）。
  它防的是一条 cancel 撞上 `#drain` 取件的窄竞态，代码注释里有完整说明。

**拉取成功要顺手作废并重建盘点缓存。** 列表页有一份 TTL 快照，不作废的话
"拉完切回去搜不到刚拉的镜像"（真实反馈）。两步缺一不可：`invalidate()` 让这期间的请求
不再拿旧快照，后台 `get({force:true})` 让用户切过去时数据已就绪。
连拉多个要**合并成一轮**（一次盘点按 tag 数打上百个请求），跑的过程中又完成就跑完再补一轮。
**失败 / 取消不触发**（不会留下新 tag），**盘点失败不能影响拉取结果**。
...
- **它必须自己钉住"模拟的系统外观"**（`Emulation.setEmulatedMedia` → `prefers-color-scheme: light`）。
  首屏主题脚本是"先读 localStorage、读不到才退回系统偏好"，所以
  "初始是浅色（未设置过选择时）"这条实际依赖**跑验证那台机器**的外观 ——
  开发机切成深色之后，一整批主题断言会无端翻红，而代码一个字没改（真实踩到，
  当时先怀疑是自己改坏了主题，白查一轮）。钉住之后，结果只反映应用逻辑；
  顺带让"刷新后仍是深色"变成真的在验 **localStorage 压过系统偏好**。
  > 通用教训：**断言不要依赖跑它的机器状态**（系统外观、时区、语言、已有数据）。
  > 外部依赖要显式钉住，否则"绿"只说明这台机器当时恰好合适。
